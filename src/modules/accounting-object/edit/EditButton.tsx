...
import {
  getAccountingObjectByIdApi,
  IUpdateAccountingObjectReq,
  updateAccountingObjectApi,
...
const EditButton = ({ id }: EditButtonProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      getAccountingObjectByIdApi(id).then((res) => {
        const initialValues: IUpdateAccountingObjectReq = {
          name: res.name,
          status: res.status,
        };
  ...
  const onFinish: FormProps["onFinish"] = (values) => {
    const data: IUpdateAccountingObjectReq = { ...values };
    updateAccountingObjectApi(id, data)
      .then(() => {
  ...
  return (
    <>
      ...
      <Modal
        title="Update Accounting Object"
        open={isOpen}
        onOk={handleOk}
        onCancel={onClose}
      >
